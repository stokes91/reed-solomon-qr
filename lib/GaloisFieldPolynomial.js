...
const { Zero, One, Multiply, Add, Invert, Size } = require('./GaloisField');
...
class GaloisFieldPolynomial {
  ...
  divideByScalar(coefficient) {
    const inverse = Invert(coefficient);

    for (let l = this.coefficients.length; l--;) {
      this.coefficients[l] = Multiply(this.coefficients[l], inverse);
    }
...
GaloisFieldPolynomial.oldMonomial = (degree, scale) => {
  const that = new GaloisFieldPolynomial();
