...
class ReedSolomonEncoder {
  encode(buffer) {
    const output = Buffer.alloc(buffer.byteLength + this.coefficients.length);
    buffer.copy(output, 0, 0, buffer.byteLength);

    const ecc = new Array(this.coefficients.length).fill(0);

    for (let i = 0; i < buffer.byteLength; i++) {
      ecc.push(0);
      const factor = Add(buffer[i], ecc.shift());
      for (var j = 0; j < this.coefficients.length; j++)
        ecc[j] = Add(ecc[j], Multiply(this.coefficients[j], factor));
    }

    ecc.forEach((that, i) => {
      output.writeUInt8(that, buffer.byteLength + i);
    });
